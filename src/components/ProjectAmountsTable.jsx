...
//   const isBudPlan = planType && (planType.toUpperCase() === 'BUD' || planType.toUpperCase() === 'NBBUD')
// const isFieldEditable = (planType && (planType.toUpperCase() === 'BUD' || planType.toUpperCase() === 'NBBUD')) || (planType && planType.toUpperCase() === 'EAC')
...
//                     setEmployeeNonLaborAccounts([]);
//                     setSubContractorNonLaborAccounts([]);
//                     // Reset maxKbdSuffix to current max from employees
//                     const existingKbdIds = employees
//                       ?.map((emp) => emp.emple?.emplId)
//                       .filter((id) => id && id.startsWith("KBD"));
//                     const suffixes = existingKbdIds.map((id) => {
//                       const match = id.match(/^KBD(\d{3})$/);
//                       return match ? parseInt(match[1], 10) : 0;
//                     });
//                     const maxSuffix = suffixes.length
//                       ? Math.max(...suffixes)
//                       : 0;
//                     setMaxKbdSuffix(maxSuffix);
//                   } else {
